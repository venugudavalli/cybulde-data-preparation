...
class GCPDaskClusterConfig(DaskClusterConfig):
    ...
    extra_bootstrap: Optional[list[str]] = field(
        default_factory=lambda: ["gcloud auth configure-docker --quiet europe-west2-docker.pkg.dev"]
    )
    ngpu: Optional[int] = 0
    gpu_type: Optional[str] = None
    filesystem_size: int = 50
    ...
    service_account_credentials: Optional[dict[str, str]] = None
