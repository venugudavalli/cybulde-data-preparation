...
def config_args_parser() -> argparse.Namespace:
    ...
    parser.add_argument("--config-name", type=str, required=True, help="Name of teh config file")
    parser.add_argument("--overrides", nargs="*", default=[], help="Hydra config overrised")
...
def custom_instantiate(config: Any) -> Any:
    ...
    if "_target_" not in config_as_dict:
        raise ValueError("Config does not have key _target_")
    ...
    splitted_target = _target_ = _target_.split(".")
